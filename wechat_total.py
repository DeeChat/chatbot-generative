...
# ----------- retrival-based model-------------------#
# get response
logging.info('Start load responses')
responses = []
with open(config2.DECODER_DATA_PATH) as f:
    for line in f.readlines():
        responses.append(line)

# load dictionary and corpus
logging.info('Start load dictionary')
dictionary = corpora.Dictionary.load(config2.TRAIN_DICT)

# build model
logging.info('Start load model')
tfidf = models.TfidfModel.load(config2.TFIDF_MODEL)
lsi = models.LsiModel.load(config2.LSI_MODEL)


# load corpus to LSI and index it
index = similarities.MatrixSimilarity.load(config2.TRAIN_INDEX)
...
def wechat(msg):
    ...
    '''
    if msg['Type'] == 'Text':
        response1 = wechat_text(msg)
        response2 = wechat_retrival(msg)
        response = "generate:" + str(response1) + '\n' + 'retrival:' + str(response2)
...
def wechat_retrival(msg):
    text = msg['Text']
    vec_bow = dictionary.doc2bow(text.split())
    vec_lsi = lsi[vec_bow]
    sims = index[vec_lsi]
    sims = sorted(enumerate(sims), key=lambda item:item[1], reverse=True)
    response_list = [i[0] for i in sims[:10]]
    response_index = random.choice(response_list)
